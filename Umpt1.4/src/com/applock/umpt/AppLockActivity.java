		...
		lv.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,int position, long id) {
				...
				if(dao.find(packName)){  //如果在数据库内找到改包名 已经加锁
					//dao.delete(packName);
					//通过内容提供者删除
					getContentResolver().delete(Uri.parse("content://cn.test.applockprovider/delete"), null,new String[]{packName});
					...
					getContentResolver().insert(Uri.parse("content://cn.test.applockprovider/insert"), values);
